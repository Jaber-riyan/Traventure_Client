...
const PackageDetails = () => {
    ...
    const handleBookingSubmit = async (data) => {
        ...
        if (response.data.data.insertedId) {
            ...
            toast.error("Something went wrong to booking!");
        }
    ...
    return (
        <div className='dark:bg-gray-900'>
            ...
            <div className='pt-28 pb-28 md:w-[87%] mx-auto w-[90%]'>
                ...
                <div>
                    <Carousel className="rounded-lg" autoPlay={true} interval={2000} infiniteLoop={true}>
                        <div className='relative'>
                            <div className='absolute inset-0 dark:bg-black/40 bg-black/20 z-10'></div> {/* Dark overlay */}
                            <img className='w-full h-full object-cover' src={placePhoto1} alt="Image 1" />
                        </div>
                        <div className='relative'>
                            <div className='absolute inset-0 dark:bg-black/40 bg-black/20 z-10'></div> {/* Dark overlay */}
                            <img className='w-full h-full object-cover' src={placePhoto2} alt="Image 2" />
                        </div>
                        <div className='relative'>
                            <div className='absolute inset-0 dark:bg-black/40 bg-black/20 z-10'></div> {/* Dark overlay */}
                            <img className='w-full h-full object-cover' src={placePhoto3} alt="Image 3" />
                        </div>

                    </Carousel>
                </div>
                {/* content  */}
                <div className="p-4 text-gray-800 dark:text-gray-300 flex flex-col justify-between mb-10">
                    <h3 className="text-4xl font-black mb-4">{name}</h3>
                    <p className="text-xl text-gray-600 dark:text-gray-400 mt-2 leading-snug">
                ...
                <div className='space-y-5 mb-16'>
                    {
                        planData?.length && planData?.map((plan, index) => {
                            return (
                                <div key={index} className='bg-gray-200 dark:bg-gray-800 shadow-xl p-9 rounded-lg'>
                                    ...
                                    <p className='text-black/60 dark:text-gray-300 font-semibold'>{plan.split("|")[1]}</p>
                                </div>
                            );
                        })
                    }
